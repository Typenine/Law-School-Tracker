...
function majorAssignment(task: Task) {
  return /(memo|brief|paper|essay|presentation|oral argument|project)/i.test(task.title);
}
...
export default function CurrentTaskCard({ task, courses, refresh }: { task: Task; courses: Course[]; refresh: () => Promise<void> }) {
  ...
  const courseRecord = courses.find(item => item.id === task.courseId) || courses.find(item => item.title === task.course);
  const overdue = task.status !== 'done' && dateKey(task.dueDate) < todayKey();
  const planned = task.tags?.includes('assignment-plan-created');
  ...
  async function saveEdit(event: FormEvent) {
    ...
    const selected = courses.find(item => item.title === course);
    await update({ title: title.trim(), course: course || null, courseId: selected?.id || null, dueDate: new Date(`${dueDate}T23:59:59`).toISOString() });
    setEditing(false);
  }
  ...
  async function buildPlan() {
    ...
    setWorking(true);
    try {
      for (const milestone of assignmentMilestones(task.title, task.dueDate)) {
        await tasksClient.create({
          title: milestone.title,
          course: task.course || null,
          courseId: task.courseId || courseRecord?.id || null,
          ...
          dependsOn: task.id,
          tags: ['assignment-plan', `assignment-parent:${task.id}`, milestone.tag],
        }, { silent: true });
      }
      await tasksClient.update(task.id, { tags: Array.from(new Set([...(task.tags || []), 'assignment-plan-created'])) }, { silent: true });
      await refresh();
    } finally { setWorking(false); }
  }
