...
export default function CourseTimelineClient() {
  ...
  const weeks = useMemo(() => {
    ...
    for (let cursor = new Date(start); cursor <= end && result.length < 24; cursor = addDays(cursor, 7)) {
      ...
      for (let offset = 0; offset < 7; offset++) {
        ...
        for (const block of courseBlocks(course)) if (block.days.includes(date.getDay())) meetings.push({ date, ...block });
      }
  ...
  if (coursesLoading || tasksLoading) return <div className="rounded-xl border border-slate-700 p-6 text-slate-400">Loading timeline…</div>;
