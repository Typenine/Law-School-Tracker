...
import { recoveryOverride, recoveryReason, type RecoveryCategory } from '@/lib/recoveryWorkflow';
...
type Category = RecoveryCategory;
...
function classify(task: Task, examDays?: number | null): Category {
  const manual = recoveryOverride(task);
...
export default function RecoveryPage() {
  ...
  const courseById = new Map(activeCourses.map(course => [course.id, course] as const));
  const courseByName = new Map(activeCourses.map(course => [course.title.toLowerCase(), course] as const));
  const examByCourseId = new Map(activeCourses.map(course => [course.id, examDaysRemaining(workspaces[course.id]?.examDate)] as const));
  const examByCourseName = new Map(activeCourses.map(course => [course.title.toLowerCase(), examDaysRemaining(workspaces[course.id]?.examDate)] as const));
  const examDaysFor = (task: Task) => task.courseId ? examByCourseId.get(task.courseId) : examByCourseName.get((task.course || '').toLowerCase());
  const open = useMemo(() => tasks.filter(task => isActiveTask(task) && task.status !== 'done' && (!currentTerm || task.term === currentTerm)).sort((a, b) => a.dueDate.localeCompare(b.dueDate)), [tasks, currentTerm]);
  const grouped = useMemo(() => {
    const result: Record<Category, Task[]> = { must: [], skim: [], ask: [], defer: [], drop: [] };
    for (const task of open) result[classify(task, examDaysFor(task))].push(task);
    return result;
  }, [open, workspaces, activeCourses]);
  const plan = useMemo(() => {
    const selected: Array<{ task: Task; mode: 'complete' | 'skim'; minutes: number }> = [];
    let remaining = availableMinutes;
    for (const task of [...grouped.must, ...grouped.skim]) {
      if (remaining < 15) break;
      const mode = classify(task, examDaysFor(task)) === 'skim' ? 'skim' : 'complete';
  ...
  async function setOverride(task: Task, category: Category | '') {
    setWorkingId(task.id);
    const tags = (task.tags || []).filter(tag => !tag.startsWith('recovery-override:'));
    if (category) tags.push(`recovery-override:${category}`);
    try { await tasksClient.update(task.id, { tags }, { silent: true }); await refresh(); } finally { setWorkingId(''); }
  }
  ...
  async function ask(task: Task) {
    const course = task.courseId ? courseById.get(task.courseId) : courseByName.get((task.course || '').toLowerCase());
    if (!course) return;
