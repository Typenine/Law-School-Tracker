...
function CourseCard({ course, workspaceMap, tasks, currentTerm, onEdit }: { course: Course; workspaceMap: CourseWorkspaceMap; tasks: any[]; currentTerm?: string | null; onEdit: (course: Course) => void; }) {
  ...
  return <article className="rounded-xl border border-slate-700 bg-slate-900/45 p-5">
    <div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex items-center gap-3"><span className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: course.color || '#64748b' }} /><h3 className="truncate text-lg font-semibold text-slate-100">{course.title}</h3></div><p className="mt-1 text-sm text-slate-500">{[course.code, course.instructor].filter(Boolean).join(' · ') || 'Course details not set'}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-medium ${tone}`}>{status}</span></div>
    <div className="mt-4 grid gap-2 sm:grid-cols-2"><div className="rounded-lg bg-slate-950/45 p-3"><p className="text-[11px] uppercase tracking-wide text-slate-600">Next class</p><p className="mt-1 text-sm font-medium text-slate-200">{nextClass ? formatClass(nextClass.start) : 'Schedule not set'}</p>{nextClass?.location ? <p className="mt-1 text-xs text-slate-500">{nextClass.location}</p> : null}</div><div className="rounded-lg bg-slate-950/45 p-3"><p className="text-[11px] uppercase tracking-wide text-slate-600">Next task</p><p className="mt-1 line-clamp-2 text-sm font-medium text-slate-200">{nextTask?.title || 'No open work'}</p>{nextTask ? <p className="mt-1 text-xs text-slate-500">Due {formatDue(nextTask.dueDate)}</p> : null}</div></div>
    <div className="mt-3 grid grid-cols-3 gap-2 text-center"><div className="rounded-lg border border-slate-800 p-2"><p className={`text-lg font-semibold ${overdue.length ? 'text-rose-300' : 'text-slate-100'}`}>{open.length}</p><p className="text-[11px] text-slate-500">Open</p></div><div className="rounded-lg border border-slate-800 p-2"><p className="text-lg font-semibold text-slate-100">{workspace.outlineProgress || 0}%</p><p className="text-[11px] text-slate-500">Outline</p></div><div className="rounded-lg border border-slate-800 p-2"><p className="text-lg font-semibold text-slate-100">{examDays === null ? '—' : Math.max(0, examDays)}</p><p className="text-[11px] text-slate-500">Exam days</p></div></div>
    <div className="mt-4 flex flex-wrap gap-2"><Link href={`/courses/${course.id}`} className="rounded-lg bg-emerald-500 px-3 py-2 text-sm font-semibold text-slate-950">Workspace</Link><Link href={`/course-timeline?course=${course.id}`} className="rounded-lg border border-violet-500/40 px-3 py-2 text-sm text-violet-300">Timeline</Link><Link href={`/tasks?course=${encodeURIComponent(course.title)}`} className="rounded-lg border border-slate-600 px-3 py-2 text-sm text-slate-200 hover:bg-slate-800">Tasks</Link><button onClick={() => onEdit(course)} className="rounded-lg border border-slate-600 px-3 py-2 text-sm text-slate-200 hover:bg-slate-800">Edit</button></div>
...
export default function CoursesPage() {
  ...
  useEffect(() => { (async () => { try { const data = await apiFetch<{ settings: Record<string, any> }>(`/api/settings?keys=${COURSE_WORKSPACES_KEY}`); setWorkspaceMap((data.settings?.[COURSE_WORKSPACES_KEY] || {}) as CourseWorkspaceMap); } catch {} })(); }, []);
  ...
  const linkedCount = activeCourses.filter((course) => { const workspace = workspaceMap[course.id] || {}; return safeUrl(workspace.notesUrl) && safeUrl(workspace.outlineUrl); }).length;
  ...
  return <main className="space-y-6">
    <section className="rounded-2xl border border-slate-700/70 bg-gradient-to-br from-slate-900 to-slate-950 p-6"><div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-sm font-medium text-emerald-300">Course workspaces</p><h2 className="mt-1 text-2xl font-semibold text-slate-100">Each course should open the work, not report statistics</h2><p className="mt-2 max-w-3xl text-sm text-slate-400">Courses center class preparation, Drive documents, weekly timelines, outline maintenance, and exam readiness.</p></div><button onClick={() => setShowAdd(true)} className="rounded-lg bg-emerald-500 px-4 py-2.5 font-semibold text-slate-950">Add course</button></div></section>
    <section className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-slate-700 bg-slate-900/45 p-4"><p className="text-xs uppercase text-slate-500">Active semester</p><p className="mt-2 text-xl font-semibold text-slate-100">{activeSemester?.name || 'Not set'}</p></div><div className="rounded-xl border border-slate-700 bg-slate-900/45 p-4"><p className="text-xs uppercase text-slate-500">Drive ready</p><p className="mt-2 text-xl font-semibold text-emerald-300">{linkedCount}/{activeCourses.length}</p><p className="text-xs text-slate-500">notes and outline linked</p></div><div className="rounded-xl border border-slate-700 bg-slate-900/45 p-4"><p className="text-xs uppercase text-slate-500">Need attention</p><p className="mt-2 text-xl font-semibold text-rose-300">{overdueCourses}</p><p className="text-xs text-slate-500">courses with overdue work</p></div></section>
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold text-slate-100">{activeSemester?.name || 'Current courses'}</h2><p className="text-sm text-slate-500">Open a course to work from its documents and assignments.</p></div><div className="flex gap-2"><Link href="/wizard" className="rounded-lg border border-slate-600 px-3 py-2 text-sm text-slate-200">Import syllabus</Link><Link href="/semester" className="rounded-lg border border-slate-600 px-3 py-2 text-sm text-slate-200">Term setup</Link></div></div>
