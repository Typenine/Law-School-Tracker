...
import { buildOutlineProposal } from '@/lib/academicWorkflow';
...
export default function OutlineUpdatesPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      for (const course of activeCourses) {
        ...
        if (exists) continue;
        const completed = tasks.filter(task => task.status === 'done' && (!currentTerm || task.term === currentTerm) && (task.course || '').toLowerCase() === course.title.toLowerCase());
  ...
  async function approve(courseId: string, courseTitle: string, proposal: OutlineProposal) {
    ...
    try {
      ...
      await tasksClient.create({
        title: `Apply approved weekly outline update: ${courseTitle}`,
        course: courseTitle,
        ...
        tags: ['outline-proposal', proposal.weekStart],
      }, { silent: true });
  ...
  return <main className="space-y-6">
    <section className="rounded-2xl border border-violet-500/30 bg-gradient-to-br from-violet-950/30 to-slate-950 p-6"><p className="text-sm font-medium text-violet-300">Weekly outline drafting</p><h2 className="mt-1 text-2xl font-semibold text-slate-100">Turn the week into reviewable outline additions</h2><p className="mt-2 max-w-3xl text-sm text-slate-400">Drafts are generated from internal class captures, syllabus topics, completed work, and unresolved questions. Nothing changes an outline until you approve it.</p></section>
