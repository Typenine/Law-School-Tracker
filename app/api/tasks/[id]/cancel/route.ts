...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const meta = parsed.data.reactivate ? await reactivateTask((await context.params).id) : await cancelTask((await context.params).id);
    if (!parsed.data.reactivate) await clearStoredTaskTimer((await context.params).id);
