...
import { dueForReview, normalizeRuleBank } from "@/lib/ruleBank";
...
export default function TodayPage() {
  ...
  const smartRecommendation = useMemo(() => {
    ...
    const remainingCapacity = Math.max(0, availableToday - loggedToday);
    const ranked = plannedTasks.map(item => {
      const task = item.task;
      const due = chicagoYmd(task.dueDate);
      const diff = Math.round((new Date(`${due}T12:00:00`).getTime() - new Date(`${today}T12:00:00`).getTime()) / 86400000);
      let score = 0;
      let reason = "Best fit for today";
      if (diff < 0) { score += 120; reason = "Overdue"; }
      else if (diff === 0) { score += 100; reason = "Due today"; }
      else if (diff === 1) { score += 70; reason = "Due tomorrow"; }
      else if (diff === 2) { score += 45; reason = "Due in 2 days"; }
      else if (diff <= 4) { score += 25; reason = `Due in ${diff} days`; }
      const priority = Number(task.priority);
      if (priority === 1) { score += 35; if (diff > 2) reason = "Highest priority"; }
      else if (priority === 2) score += 25;
      else if (priority === 3) score += 12;
      const logged = sessionMinutesByTask.get(task.id) || 0;
      if (logged > 0) { score += 12; if (diff > 2) reason = "Already in progress"; }
      if (remainingCapacity > 0 && item.minutes <= remainingCapacity) score += 8;
      if (activityLabel(task) === "reading" && diff <= 1) score += 10;
      return { item, score, reason };
    }).sort((a, b) => b.score - a.score || new Date(a.item.task.dueDate).getTime() - new Date(b.item.task.dueDate).getTime());
    return ranked[0] || null;
  }, [plannedTasks, availableToday, loggedToday, sessionMinutesByTask, today]);

  const dueRuleReviews = useMemo(() => normalizeRuleBank(settings.ruleBankV1)
    .filter(rule => dueForReview(rule, today))
    .sort((a, b) => a.nextReview.localeCompare(b.nextReview) || a.course.localeCompare(b.course))
    .slice(0, 4), [settings.ruleBankV1, today]);
  ...
  return (
    <main className="today-dashboard">
      <style jsx global>{`
        ...
        @media(max-width:760px){.today-rail{grid-template-columns:1fr}.up-card{padding:25px 21px}.up-line{align-items:flex-start}.up-line-copy{flex-wrap:wrap}.up-brand{width:34px;height:34px}.up-tools{gap:4px}.up-title{font-size:27px}.up-facts{grid-template-columns:1fr;gap:16px}.up-fact+.up-fact{padding-left:0;border-left:0}.up-actions{align-items:stretch}.timer-readout{order:5;width:100%;padding-top:5px}.dash-task{grid-template-columns:56px 4px minmax(0,1fr) 56px}.row-start{display:none}.summary-grid{grid-template-columns:1fr}.task-duration{font-size:12px}}
      `}</style>
      ...
      {loading ? <div className="loading-dash">Loading today’s plan…</div> : (
        <div className="today-grid">
            ...
            {upTask && upNext ? (
              <article className="dash-card up-card">
                <div className="up-line">
                  <div className="up-line-copy"><img className="up-brand" src="/icon.svg" alt="" aria-hidden="true" /><strong>UP NEXT</strong><span>•</span><span>{upTask.course || "Unassigned"} · {activityLabel(upTask)}</span></div>
                ...
                {smartRecommendation ? (
                  smartRecommendation.item.task.id === upTask.id
                    ? <div className="smart-rec active"><span>Recommended next · {smartRecommendation.reason}</span><span>Based on deadline, priority, progress, and today&apos;s capacity</span></div>
                    : <div className="smart-rec"><span>Suggested next: {smartRecommendation.item.task.title} · {smartRecommendation.reason}</span><button type="button" onClick={() => persistFocus(smartRecommendation.item.task.id)}>Use suggestion</button></div>
                ) : null}
          ...
          <aside className="today-rail">
            ...
            <article className="dash-card rail-card">
              <div className="rail-head"><div className="dash-eyebrow">Spaced review</div><a className="rail-link" href="/rules?review=1">{dueRuleReviews.length ? "Review now →" : "Rule Bank →"}</a></div>
              {dueRuleReviews.length ? <>{dueRuleReviews.map(rule => <div className="review-row" key={rule.id}><div className="review-topic">{rule.topic}</div><div className="review-meta">{rule.course || "Unassigned"} · due now</div></div>)}<div className="mt-4 text-xs text-slate-500">{dueRuleReviews.length} rule{dueRuleReviews.length === 1 ? "" : "s"} shown{normalizeRuleBank(settings.ruleBankV1).filter(rule => dueForReview(rule, today)).length > dueRuleReviews.length ? " · more waiting" : ""}</div></> : <div className="empty-dash">No rule reviews due today.</div>}
            </article>
