...
import { examPlanTasks } from '@/lib/academicWorkflow';
import { courseTermMatches, courseTasks, examDaysRemaining, taskKind } from '@/lib/courseWorkspace';
...
export default function ExamPage() {
  ...
  const examCourses = useMemo(() => activeCourses.map(course => {
    ...
    const prep = workspace.examPrep || {};
    return { course, workspace, matching, outline, practice, review, prep, days: examDaysRemaining(workspace.examDate) };
  ...
  async function addTask(course: Course, title: string, activity: string, dueDate?: string, notes?: string) {
    ...
    await tasksClient.create({ title, course: course.title, dueDate: due.toISOString(), status: 'todo', term: currentTerm || null, activity, notes: notes || null }, { silent: true });
  }
  ...
  async function generatePlan(course: Course, examDate: string | undefined, weakAreas: string[]) {
    ...
    try {
      const existing = new Set(tasks.filter(task => (task.course || '').toLowerCase() === course.title.toLowerCase()).map(task => task.title.toLowerCase()));
      let created = 0;
      for (const item of examPlanTasks(course.title, examDate, weakAreas)) {
        if (existing.has(item.title.toLowerCase())) continue;
        await tasksClient.create({ title: item.title, course: course.title, dueDate: item.dueDate, status: 'todo', term: currentTerm || null, activity: item.activity, estimatedMinutes: item.minutes, tags: ['exam-plan'] }, { silent: true });
        created++;
      }
  ...
  return <main className="space-y-6">
    <section className="rounded-2xl border border-amber-500/30 bg-gradient-to-br from-amber-950/30 to-slate-950 p-6"><p className="text-sm font-medium text-amber-300">Exam mode</p><h2 className="mt-1 text-2xl font-semibold text-slate-100">Build issue-spotting tools, not just study hours</h2><p className="mt-2 max-w-3xl text-sm text-slate-400">Track weak issues, rule statements, case analogies, flowchart candidates, practice results, printed-outline additions, and a countdown task plan.</p></section>
    ...
    <div className="space-y-6">{examCourses.map(({ course, workspace, outline, practice, review, prep, days }) => {
      ...
      const readiness = Math.round(((workspace.outlineProgress || 0) + Math.min(100, practiceDone * 20) + Math.min(100, reviewDone * 20)) / 3);
      return <article key={course.id} className={`rounded-xl border p-5 ${days !== null && days <= 21 && days >= 0 ? 'border-amber-500/40 bg-amber-500/5' : 'border-slate-700 bg-slate-900/45'}`}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><h2 className="text-lg font-semibold text-slate-100">{course.title}</h2><p className="mt-1 text-sm text-slate-400">{workspace.examFormat || 'Exam format not recorded'}</p></div><div className="flex gap-2"><span className="rounded-full bg-amber-500/15 px-2.5 py-1 text-xs text-amber-300">{days === null ? 'Set exam date' : days < 0 ? 'Completed' : `${days} days`}</span><span className="rounded-full bg-emerald-500/15 px-2.5 py-1 text-xs text-emerald-300">{readiness}% ready</span></div></div>
