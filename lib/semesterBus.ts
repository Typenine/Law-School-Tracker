...
if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
  try { chan = new BroadcastChannel('semesters'); } catch {}
}

const LS_KEY = 'semesterActiveAt';

export function notifySemesterChanged(): void {
  const ts = Date.now();
  if (chan) {
...
export function onSemesterChanged(cb: () => void): () => void {
  const onMsg = () => cb();
