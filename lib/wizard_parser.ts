...
function normalize(raw: string, reference: Date): Line[] {
  ...
  for (const line of filtered) {
    ...
    const join = previous
      ...
      && (/[–-]$/.test(previous.text) || ((READING.test(previous.text) || TASK.test(previous.text)) && !/[.:;!?)]$/.test(previous.text)));
    if (join) {
...
export function buildWizardPreview(rawText: string, courseHint?: string | null, options?: WizardOptions): WizardPreview {
  ...
  for (const line of lines) {
    ...
    if (startDate && (inSchedule || taskLike || readingLike || CANCELED.test(line.text))) {
